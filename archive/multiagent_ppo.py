...
# TorchRL 0.10 sends `min`/`max` from action specs, but TanhNormal may not accept them.
...
# Patch _TrajectoryPool to avoid shared_memory_ (not permitted in some sandboxes)
class _NoShmTrajectoryPool:
...
collectors._TrajectoryPool = _NoShmTrajectoryPool
...
# Devices: prefer GPU if available and not forked, else CPU
...
#
# Simple shared MLP applied per-agent (broadcast over leading dims)
...
critic_in = (
    obs_dim * env.n_agents
    if mappo
    else obs_dim
)

critic_backbone = torch.nn.Sequential(
    torch.nn.Linear(critic_in, 256),
    torch.nn.Tanh(),
    torch.nn.Linear(256, 256),
    torch.nn.Tanh(),
...
if mappo:
    class GlobalCritic(torch.nn.Module):
        ...
        def forward(self, obs):
            flat = obs.reshape(*obs.shape[:-2], -1)  # concat agents
            val = self.backbone(flat)  # (..., 1)
            # repeat value per agent
            return val.unsqueeze(-2).expand(*obs.shape[:-2], self.n_agents, 1)

    critic = TensorDictModule(
        module=GlobalCritic(critic_backbone, env.n_agents),
        in_keys=[("agents", "observation")],
...
replay_buffer = ReplayBuffer(
    storage=ListStorage(
        frames_per_batch
    ),  # simple Python list-based storage to avoid shared memory issues
    sampler=SamplerWithoutReplacement(),
    batch_size=minibatch_size,  # We will sample minibatches of this size
    writer=RoundRobinWriter(compilable=True),  # use int counters, no shared memory
)
